...
function AuthStall({ role, mode, setMode, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (mode === "login") {
        // Credentials are verified by the auth backend (BCrypt).
        const user = await api.login(emailKey, password, role);

        let customerId = null;
        if (role === "customer") {
          // The store profile id (ecom_db.customers.id) is linked to the
          // email on this browser at signup time.
          const map = JSON.parse(
            localStorage.getItem("vyapari_customer_map") || "{}"
          );
          customerId = map[emailKey] || null;

          if (!customerId) {
            throw new Error(
              "Login verified, but no store profile is linked to this email on this browser. " +
                "Please sign up on this device first (cross-device login is not supported yet)."
            );
          }
        }

        onSuccess({ role, user, customerId, email: emailKey }, role);
      } else {
        // 1) Create credentials in the auth backend (password BCrypt-hashed).
        const user = await api.signup(emailKey, password, role);

        let customerId = null;
        if (role === "customer") {
          // 2) Create the store profile in the customer backend; its id is
          //    what the cart / order / payment APIs identify the customer by.
          let profile;
          try {
            profile = await api.createCustomerProfile(
              emailKey.split("@")[0],
              emailKey
            );
          } catch (profileErr) {
            throw new Error(
              "Your login account was created, but the store profile could not be created" +
          ...
          // Remember email -> customerId so future logins on this browser work.
          const map = JSON.parse(
            localStorage.getItem("vyapari_customer_map") || "{}"
          );
          map[emailKey] = customerId;
          localStorage.setItem("vyapari_customer_map", JSON.stringify(map));
        }
...
export default function LandingAuth() {
  ...
  const handleAuthSuccess = (result, role) => {
    ...
    if (result?.email) localStorage.setItem("vyapari_email", result.email);
    if (result?.customerId) {
      localStorage.setItem("vyapari_customer_id", String(result.customerId));
    }
